...
#Combines numbers back into original number on a side
def CombineNumbers(side):
    b = False
    if (len(side) >= 2):
        for x in range(len(side)):
            for y in range(1, len(side)):
                if (x == y):
                    break
                elif (y - x == 1):
                    if ((side[x] + side[y]).isdigit()):
                        side.insert(x, side[x] + side[y])
                        side.pop(x + 1)
                        side.pop(y)
                        b = True
                        break
                elif (x - y == 1):
                    if ((side[y] + side[x]).isdigit()):
                        side.insert(y, side[y] + side[x])
                        side.remove(side[y + 1])
                        side.remove(side[x])
                        b = True
                        break
            if (b):
                side = CombineNumbers(side)
                break
    return side            
   
    
#Parentheses on a side
def Parentheses (side):
    
    
#Operations
#Includes operations on negative numbers
def Operations (side):
    side = Multiplication(side)
    side = Division(side)
    side = Addition(side)
    side = Subtraction(side)
...
#Finds side with variable
def FindVariable(side1, side2, variable):
    
    
#Isolates the variable
def Isolate(side1, side2, variable):
    sideWith = FindVariable(side1, side2, variable)
    
    
#Solve
def Solve(side1, side2, variable):
    #Formatting
    side1 = Format(side1)
    side2 = Format(side2)
    side1 = CombineNumbers(side1)
    side2 = CombineNumbers(side2)
    
    #Parentheses
    side1 = Parentheses(side1)
    side2 = Parentheses(side2)
    
    #Operations
    side1 = Operations(side1)
    side2 = Operations(side2)
    
    #Isolate the variable
    solution = Isolate(side1, side2, variable)
    return solution
    
    
#Main
side1 = input("What is on the left side of the equal sign?")
side2 = input("What is on the right side of the equal sign?")
variable = input("What variable are you solving for?")

solution = Solve(side1, side2, variable)
